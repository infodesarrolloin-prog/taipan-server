...
const GroupConnectionsPage = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const grpRes = await fetchOrThrow('/api/groups', { signal: controller.signal });
        const grpData = await grpRes.json();
        const item = grpData.find((g) => g.id === Number(id));
        if (item) {
          setGroup(item);
          setLocalAttrs({
            cutPowerCommandId: item.attributes?.cutPowerCommandId || '',
            restorePowerCommandId: item.attributes?.restorePowerCommandId || '',
            alarmOnCommandId: item.attributes?.alarmOnCommandId || '',
            alarmOffCommandId: item.attributes?.alarmOffCommandId || '',
          });
        }
      } catch {
        // ignore abort / errors
  ...
  const handleSaveAll = async () => {
    ...
    setSaving(true);
    try {
      const cleanAttrs = { ...group.attributes };
      ...
      const updatedGroup = { ...group, attributes: cleanAttrs };
      const response = await fetchOrThrow(`/api/groups/${id}`, {
        method: 'PUT',
          ...
          <Accordion defaultExpanded>
            ...
            <AccordionDetails className={classes.details}>
              <Typography variant="body2" color="textSecondary" sx={{ mb: 1 }}>
                Asigna comandos guardados a los botones de la tarjeta de estado para todos los
