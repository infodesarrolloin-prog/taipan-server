...
import { getDeviceStatusInfo } from './core/preloadImages';
...
const MapPositions = ({
  ...
  const iconScale = useAttributePreference('iconScale', desktop ? 0.75 : 1.15);
  ...
  const createFeature = useCallback(
    (devices, position, selectedPositionId) => {
      ...
      return {
        ...
        name: isPowerCut ? `${device.name} ⚠️` : device.name,
        ...
        category: isPowerCut
          ...
          ? statusInfo.category === 'arrow'
            ...
            : 'pause'
          : statusInfo.category,
        color: isPowerCut ? 'error' : isAlarmActive ? 'warning' : showStatus ? statusInfo.color : 'neutral',
        rotation: statusInfo.category === 'arrow' ? position.course : 0,
